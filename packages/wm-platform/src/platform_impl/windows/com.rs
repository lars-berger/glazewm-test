...
impl ComInit {
  ...
  pub fn service_provider(&self) -> crate::Result<&IServiceProvider> {
    self.service_provider.as_ref().ok_or(crate::Error::Platform(
      "Unable to create `IServiceProvider` instance.".into(),
    ))
  }
  ...
  pub fn application_view_collection(
    ...
    self.application_view_collection.as_ref().ok_or(
  ...
  pub fn taskbar_list(&self) -> crate::Result<&ITaskbarList2> {
    self.taskbar_list.as_ref().ok_or(crate::Error::Platform(
      "Unable to create `ITaskbarList2` instance.".into(),
    ))
  }
}
...
mod interfaces {
  ...
  use windows::core::{IUnknown, IUnknown_Vtbl, HRESULT};
